...
import type { SubscrioConfig } from "subscrio/config";
...
export async function runCatalogAccountingDemo(config: SubscrioConfig) {
  ...
  for (const [key, valueType, defaultValue] of [
    ...
    await app.products.associateFeature(p, key, { subscriptionRule: "most_generous" });
  }
  ...
  assert.equal((await app.features.getFeature(seats))!.addons[0].featureValues[seats], "3");
  assert.equal((await app.plans.getPlan(plan))!.addons[0].key, addon);
  assert.equal((await app.subscriptions.getSubscription(s))!.addons[0].quantity, 2);
  ...
  await app.features.updateFeature(meter, { meteredConfig: {
    resetPeriod: "monthly",
    usageScope: "customer",
    aggregation: "sum",
    enforcement: "soft",
  ...
  assert.equal(
    (await app.metering.getUsage(c, p, meter)).consumed,
    0,
  );
  assert.deepEqual(
    await app.metering.reportUsage(c, p, meter, 7, request),
  ...
  assert.equal(
    (await app.metering.listUsageEvents(c, p, meter)).length,
    2,
  );
  pass(
